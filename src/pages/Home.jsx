...
const Home = () => {
  ...
  const { user } = useAuth();
  const { theme } = useTheme(); // Only theme is needed now
      ...
      <main className="flex-grow flex items-center justify-center px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center py-12 max-w-4xl w-full">
          ...
          {!user && (
            ...
            >
              <motion.button
                ...
                className="relative px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium rounded-xl hover:shadow-lg transition-all group overflow-hidden"
              ...
              <motion.button
                ...
                className="relative px-8 py-4 bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-medium rounded-xl border-2 border-indigo-500/20 hover:border-indigo-500/40 hover:shadow-lg transition-all group overflow-hidden"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
          ...
          {user && (
            ...
            >
              {['Dashboard', 'Analytics', 'Profile'].map((item, i) => (
                <motion.div
                  key={item}
                  whileHover={{ y: -5 }}
                  whileTap={{ scale: 0.98 }}
